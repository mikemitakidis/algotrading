...
class G10_Hygiene(unittest.TestCase):
    ...
    def test_no_duplicate_test_class_definitions(self):
        """Guard against silent unittest class shadowing.

        Python keeps only the LAST definition when a class name is
        declared more than once in a module. Earlier definitions are
        silently discarded, which can drop or misattribute test
        methods without any error. This guard parses THIS file and
        fails if any top-level class name is defined more than once.

        Added after the M18.B12 pre-M19 cleanup, which found seven
        G7_* classes each defined twice in this file.
        """
        ...
        self.assertEqual(
            duplicates, {},
            "Duplicate top-level class definitions found in "
            f"{this_file.name} (later defs silently shadow earlier "
            f"ones, dropping/misattributing tests): {duplicates}")
    ...
    def test_no_unexpected_files_added(self):
        ...
        allowed_exact = {
            ...
            # M20.D: paper order + fill proof tests.
