...
> **Revision note (post-review correction commit).** Three issues
> raised by ChatGPT's review of the first M13.5.A commit are fixed
> in-place:
>
> 1. **Architecture contradiction removed.** §9 previously implied
>    that `bot/brokers/__init__.py` would replace the `etoro_real`
>    `ValueError` with construction of `EtoroLiveBroker`. That
>    contradicted §1.4 scanner isolation. Corrected: the `ValueError`
>    stays; the registry never returns `EtoroLiveBroker`; the only
>    constructor of `EtoroLiveBroker` is `tools/etoro_live_write.py`.
> 2. **False claim about `client_intent_id` removed.** §8.4 previously
>    stated `client_intent_id` was already a column added by M15.0.
>    It is not. M15.0 added six columns: `submitted_at`, `filled_at`,
>    `fill_price`, `fill_qty`, `cancelled_at`, `lifecycle_json`.
>    Corrected: `client_intent_id`, `nonce_digest`, and `x_request_id`
>    are stored as keys inside the existing `lifecycle_json` column.
>    No new schema migration is proposed for M13.5.B. If M13.5.B
>    discovers JSON-based dedupe is unworkable, it must come back here
>    with an amendment, not silently add a column.
> 3. **Trade history path corrected.** §2.1 previously listed the
>    OpenAPI-doc path as "verified live read-only in M13 discovery";
>    that was inaccurate — M13 verified `GET /api/v1/trading/info/trade/history?minDate=...`,
>    not the docs-listed path. Corrected to reflect both paths
>    honestly and to require M13.5.B re-verification before use.
...
| Trade history                          | GET    | M13 discovery live-verified `GET /api/v1/trading/info/trade/history?minDate=...` (read-only). The OpenAPI index lists a `trading--real/list-trading-history` page whose path may differ; **M13.5.B must re-verify the exact path against the OpenAPI JSON spec before any use, and update this row** |
...
2. `execution_intents` row inserted **before** the POST, in status
   `pending_live_write`. **No new column is added in M13.5.B.** The
   following identifiers are stored as keys inside the existing
   `lifecycle_json` column (added in M15.0):
   - `client_intent_id` — a UUID generated by the operator CLI at the
     moment the row is inserted. Recorded in `lifecycle_json` and used
     as the canonical "this command, this row" identifier.
   - `nonce_digest` — the 8-char digest the operator echoed (see
     M13.4B §10). Single-use; rejected on replay.
   - `x_request_id` — the UUID sent on the eToro POST header.
   M13.5.B tests prove the exactly-one-row property by:
   (a) inserting the row, then attempting a second insert with the
       same `client_intent_id` and asserting it is rejected via a
       guard in `bot/etoro/lifecycle.py` that scans `lifecycle_json`
       (the JSON1 SQLite extension is sufficient, or a defensive
       Python-side check against recent rows);
   (b) re-running the operator CLI with an expired nonce and asserting
       no `POST` is emitted.
3. Operator CLI exits immediately after one POST; no loop

If empirical evidence in M13.5.B shows that scanning `lifecycle_json`
is too slow or unreliable, M13.5.B may **propose** (not unilaterally
add) a real `client_intent_id` column migration. Any such proposal
arrives as a separate ChatGPT-reviewed amendment to this evidence
pack, not as a silent schema change.
...
- `bot/brokers/__init__.py` modification is **narrowed**: it adds exactly
  one branch — when policy says auto-trading is disabled for the active
  `BROKER`, wrap the constructed broker in `SignalOnlyBroker`. The
  existing `etoro_real` `ValueError` **stays in place and is not
  replaced**. The registry never constructs `EtoroLiveBroker` under any
  `BROKER` value. `BROKER=etoro_real` in normal runtime continues to
  fail loudly with `ValueError`. The only code path that constructs
  `EtoroLiveBroker` is the operator CLI `tools/etoro_live_write.py`,
  which imports the live broker class directly and is not imported
  anywhere on the scanner / main / strategy / risk path. This preserves
  the §1.4 scanner-isolation guarantee.
...
- `bot/brokers/__init__.py` **MOD** — add one `SignalOnlyBroker` wrap
  branch only. The `etoro_real` `ValueError` is **kept unchanged**.
  Registry never returns `EtoroLiveBroker`.
