...
a promotable model; trainability gates unchanged so fixtures still train) is DONE. Suite at 668 OK / skipped=3
...
M18.B.11 model-readiness reporter is DONE: bot/ml/readiness.py (assess_readiness, pure/read-only) + thin `cli readiness --model-id` subcommand (B9 envelope). ADVISORY ONLY (readiness_is_advisory=true, promotion_gate=false; promotion still owned by B4/B8). Consumes stored evaluation_report.json; does not recompute metrics or edit evaluator/report/trainer/registry. Reports overfit gap (train-minus-val/test on PR-AUC & ROC-AUC, warn 0.15), stored-calibration verdict (ECE/MCE/Brier; predict_time_calibration_applied=false disclaimer), baseline verdict (read-if-present, else honestly unavailable), regime/symbol coverage (thin segments=warnings), production-thinness status, aggregate ready+reasons/warnings/limitations. Timing not gated. G11_Readiness 13 tests (synthetic reports + one real fixture CLI path), no side effects, AST no-live-import check. Deferred B11.x/M21: cross-fold feature-importance stability + speed thresholds.

M18.B.12 (final docs/acceptance/merge-prep) is DONE: added docs/M18_ACCEPTANCE.md (consolidated acceptance + proof table + known limitations + fast-forward merge plan, NOT executed); reconciled MILESTONE_STATUS.md (removed the false 'M17 has not started'; added rows: M17 CLOSED on main, M18 foundation-complete on branch and NOT merged), ROADMAP.md (current-state banner), docs/PROJECT_STATUS_RECONCILIATION.md (M17/M18 update note), docs/M18_status.md (brought current to 668 OK / B12). No code change: the B10 audit command-string 'HEAD-- bot/risk.py' was confirmed a terminal display-wrapping artifact only — the real string is 'git diff --numstat origin/main HEAD -- bot/risk.py' (correct), so bot/ml/audit.py left untouched. State: origin/main a8d8ca4 (M17.B.closeout); branch 0 behind (exact ahead count not pinned in docs since docs-only commits change it — verify live with `git rev-list --left-right --count origin/main...HEAD` before merge); M18 not on main. Merge plan: fast-forward only, no squash, no merge commit, no force push, after explicit operator approval. Known limitations recorded: B9.C deferred (AssemblerResult persistence/reload), B3 gap (predict-time calibration not applied), B11.x/M21 (cross-fold feature-importance stability + speed thresholds), duplicate G7 classes (latent test-organisation trap, not a correctness blocker — not deduped in B12), M18 is not live trading and not M19 scoring, readiness advisory not a gate.

M18.B.10 audit/safety runner is DONE: new bot/ml/audit.py AuditRunner (READ-ONLY; injected git_runner/test_runner so unit tests use fakes, no real git mutation or full-suite run) + thin `cli audit` subcommand reusing the B9 envelope. Modes: static (branch/HEAD, git status, requirements diff vs origin/main, data/ml tracked+local, protected-path diff for main.py/bot/data.py/bot/risk.py/bot/scanner.py/bot/strategy.py/dashboard/app.py, bot/data.py content-SHA256 pin, forbidden execution-pattern scan, syntax via builtin compile so no .pyc written), hygiene=default (static + invoke G10_Hygiene of M18 and M17.B via unittest, not reimplemented), full=opt-in (+ M17.B full regression). --json => B9 envelope (failure to stderr), non-zero exit on any failed check, no traceback unless --debug. signals.db/sqlite3 deliberately NOT in the static forbidden set (flywheel reader legitimately reads it; nuance delegated to G10). G10_AuditRunner: 16 tests, all fake-injected, proves clean->ok + each failure path + no files created + AST-based no-live-import check.
...
M18.B.8 (artifact consistency) is DONE: training_metadata.json now persists dataset_hash/dataset_manifest_hash/repro_hash_v2/missingness_policy_hash + n_train/n_val/n_test + training_X/y row & column counts (artifact_schema_version=2); Registry.verify_artifact_consistency(model_id) returns JSON-safe {consistent,problems} and fail-closed flags missing train_outputs/metadata/X/y, n_features<->training_X width<->feature_columns mismatch, training_y length mismatch, base+indicators<->model columns mismatch, and metadata-vs-entry dataset-hash mismatch; promote_to_current runs it FIRST and raises PromotionBlockedError(integrity) on any inconsistency (--force cannot override). Predict path already validates base columns + derives indicators (B.5). Post-review hardening: verify_artifact_consistency now READS and validates train_outputs (dataset_hash/repro_hash/n_features/n_train/n_val/n_test cross-checks vs metadata+entry+training_X), evaluation_report (present/readable + dataset_hash/model_type identity + n_train/n_val/n_test vs metadata and train_outputs), and training_feature_summary (keys==model cols, q01/q99 per feature), plus persisted training_X_rows/columns/training_y_rows vs actual parquet shapes and schema-v2 required-field presence. Limitation: 'model artifact' = deterministic-refit source, not a frozen-weights blob; no separate per-artifact hashes yet.
with these added.
